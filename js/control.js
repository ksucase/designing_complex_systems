// See hlvoorhees’s block #5986172 July 12, 2013
// 3D scatter plot using d3, x3dom
// http://bl.ocks.org/hlvoorhees/5986172#scatter_plot_3d_demo.js
...
//     .attr( "position", [8, 4, 15])
     .attr( "centerOfRotation", [5, 5, 5])
     .attr( "fieldOfView", [-5, -5, 20, 20])
     .attr( "orientation", [-0.5, 1, 0.2, 1.12*Math.PI/4])
     .attr( "position", [8, 4, 15])


  var rows = initializeDataGrid();
  var axisRange = [[0, 16], [0,16],[0, 16]];
  var scales = [];
  var initialDuration = 0;
  var defaultDuration = 800*2;
  var ease = 'linear';
  var time = 0;
  var axisKeys = ["x", "y", "z"];
  ...
  function initializePlot() {
    initializeAxis(0, axisRange[0]);
    initializeAxis(1, axisRange[1]);
    initializeAxis(2, axisRange[2]);
  }

  function initializeAxis( axisIndex , axisRange)
  {
    var key = axisKeys[axisIndex];
    drawAxis( axisIndex, key, initialDuration, axisRange );
  ...
  function drawAxis( axisIndex, key, duration, axisRange ) {

    var scale = d3.scale.linear()
      .domain( [0,16] ) // demo data range
      .range( axisRange )
...
//    }
 // 16 homes x = level = 0
for (var z=0; z<16; z+=1) {
	rows.push({x: 0, y: 0, z: z});
}

  //  4 neighborhoods x = level = 1
for (var z=0; z<16; z+=4) {
	rows.push({x: 1*2, y: 0, z: z});
}

//  laterals x = level = 2
for (var z=0; z<16; z+=4) {
	rows.push({x: 2*2, y: 0, z: z});
}

//  feeders x = level = 3
for (var z=0; z<1; z+=4) {
   	rows.push({x: 3*2, y: 0, z: z});
   }

   //  substation x = level = 4
   for (var z=0; z<1; z+=1) {
   	rows.push({x: 4*2, y: 0, z: z});
   }
  ...
  function updateData() {
    ...
    if ( x3d.node() && x3d.node().runtime ) {
      ...
      for (var r=0; r<rows.length; ++r) {
        ...
        var y = rows[r].y;
        if (x== 0){
        	rows[r].y = Math.abs(5*( Math.sin(0.5*x + time) * Math.cos(0.25*z + time)));
        }
        else if (x==2) {
			//rows[r].y = rows[r*2/1+0].y + rows[r*2/1+1].y + rows[r*2/1+2].y + rows[r*2/1+3].y;
		}
		else if (x == 4){
			//rows[r].y = rows[r*2/1+0].y + rows[r*2/1+1].y + rows[r*2/1+2].y + rows[r*2/1+3].y;
		}
